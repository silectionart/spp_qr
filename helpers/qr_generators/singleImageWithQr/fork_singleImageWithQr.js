...
const genImage = async (xdata) => {
  ...
  if (data['VPA'] && data['qr code'] && data['Barcode']) {
    ...
    // end of vars
    let qr = await simpleQRGenerator(qrCode);
    await require('fs').writeFile('images/generated/' + genData.outputFolder + '/qr/' + batch + '.png', qr, (err) => {
      if (err) {
        console.log(err);
      }
    });
    await blitSingleSided(uid, vpa, batch, genData);
