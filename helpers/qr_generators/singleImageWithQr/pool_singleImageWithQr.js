...
const generateFromExcel = async genData => {
  ...
  if (newJob) {
    ...
    await fsExtra.ensureDir(
      "images/generated/" + genData.outputFolder + "/barcode"
    );
    await fsExtra.ensureDir(
      "images/generated/" + genData.outputFolder + "/sticker1sided"
    );
  ...
  const Pool = new pool(
    path.join(__dirname, "./fork_singleImageWithQr.js"),
    null,
    null,
    {}
  );
  ...
  let i;
  if (genData.startRow == 0) {
    i = 0;
  } else {
    i = genData.startRow - 1;
  }
